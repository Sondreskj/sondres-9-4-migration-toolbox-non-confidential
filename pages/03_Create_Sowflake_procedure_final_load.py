...
def generate_snowflake_procedure(df, procedure_name, author, schema_from, schema_to):
    # Update the procedure signature and return type
    procedure_code = f"CREATE OR REPLACE PROCEDURE {schema_to}.{procedure_name}_RAW_TO_FOUNDATION()\n"
    procedure_code += "RETURNS VARCHAR(16777216)\nLANGUAGE SQL\n"
    procedure_code += f"COMMENT = 'Loading of table {procedure_name} based on INSERT_HERE'\n"
    # Update the EXECUTE AS clause
    procedure_code += "EXECUTE AS OWNER\nAS '\n"

    # Add a comment and the SQL command
    procedure_code += f"-- Overwrites data in {procedure_name} with data from INSERT_HERE\n"
    procedure_code += f"INSERT OVERWRITE INTO {schema_to}.{procedure_name}\n(\n"

    for _, row in df.iterrows():
        column_name = row['column_name'].upper()
        if column_name:  # add column only if name is not None
            procedure_code += f"    {column_name},\n"

    procedure_code = procedure_code.rstrip(',\n')  # remove trailing comma
    procedure_code += "\n)\n SELECT\n"
...
# Streamlit code for generating the Snowflake procedure
st.title('Snowflake Procedure Generator ')
st.write("""
This page will take raw SAS code from the Table Loader function, and create a INSERT OVERWRITE INTO statment you can use as the final step in SP or VIEWS.
""")
...
if st.button('Convert Table loader code'):
    if sas_code:
        ...
        schema_from = st.text_input("Enter the source schema name", value='NOR_OPS_RAW')
        ...
        if procedure_name and author:
            with st.expander("Create Table Statement"):
                snowflake_procedure = generate_snowflake_procedure(df_updated, procedure_name, author, schema_from, schema_to)
                st.code(snowflake_procedure, language='sql')